...
import { useAdminGetExecutionsQuery } from '@/entities/execution'
...
export function AdminExecutionsPage() {
  ...
  const { data, isLoading } = useAdminGetExecutionsQuery(
    { status: status || undefined, search: search.trim() || undefined, sort, limit: PAGE, offset },
    { pollingInterval: 30000 },
  )

  const empty =
    status === 'submitted' && !search.trim()
      ? t('admin.executions.empty')
      : t('admin.executions.emptyFiltered')

  return (
    <div>
      <h1 className="text-2xl font-bold mb-4">{t('admin.executions.title')}</h1>
      ...
      <div className="flex gap-2 mb-3 flex-wrap">
        {STATUSES.map((s) => (
          ...
          >
            {s ? t(`statuses.${s}`) : t('admin.executions.filterAll')}
          </Button>
        ))}
      </div>
      ...
      <div className="flex gap-2 mb-4 flex-wrap">
        <Input
          className="max-w-sm"
          placeholder={t('admin.executions.searchUser')}
          value={search}
          onChange={(e) => {
        ...
        <SimpleSelect
          ...
          options={[
            { value: 'newest', label: t('admin.executions.sortNewest') },
            { value: 'oldest', label: t('admin.executions.sortOldest') },
          ]}
        />
      </div>
